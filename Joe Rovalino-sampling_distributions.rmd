...
JR Anwser:
The population is right skewed. There are large portions of the histogram at 500-2000. But starts to right skew with 2000 to 4000 area.
...
JR Answer: I would expect it to be simliar but not exact as it is only taking 50 of the total 2930. For example the mean is 1500 for the popoulation but in my sample it was 1432 - so very simliar but not same. 
...
```
JR Answer;
The sample 2 mean was again not same as sample1 but close to it - 1432. As we increased the sample size to the popultation I would expect we would get more accurate to the mean of the population
...
JR Answer:
There are 5000 elements. The sampling distribution takes 50 samples and repeats it 5000 times and stores the mean of each iteration. I don't expect for it to change very much. They won't be exact but as the sample gets larger I would think that it would get closer to the mean of the entire population. 
...
```
There are 100 elements and each element is the sample mean of a sample of 50 houses.
...
The Center gets more dense with more of the results in the center. There is less spred with a larger sample size. The sample with 10 was much wider in results or more spread out vs. the sampel with 100 was more compressed and less spread. 
* * *
## On your own
...
sample_means50_p <- mean(samp_price)
sample_means50_p         
```
Jr Answer:
When I ran it for a sample of 50 - I received a mean of of 178,500
...
```{r loop_jr1, eval=TRUE}
sample_means_50 <- rep(NA, 5000)

for(i in 1:5000){
   samp_price <- sample(price, 50)
   sample_means_50[i] <- mean(samp_price)
}
par(mfrow = c(1, 1))
hist(sample_means_50)

pop_price_mean <- mean(price)
pop_price_mean

```
The sample looks like a normal distribution with the mean around 180K. This is also the mean for the population. The 5000 iterations of 50 samples was enough to get very close to the population mean. 
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    ```{r loop-jr2, eval=TRUE}
sample_means_150 <- rep(NA, 5000)

for(i in 1:5000){
   samp_price_150 <- sample(price, 150)
   sample_means_150[i] <- mean(samp_price_150)
}
par(mfrow = c(1, 1))
hist(sample_means_150)
    
```
JR Answer: 
I find the mean is same around 180 but the data is less spread out. There is less tails. Most of the data falls in the 175 to 185 range. 
...
The larger sample (150) would be less spread out then a sample of 50. I would prefer a small spread to be more accurate. 